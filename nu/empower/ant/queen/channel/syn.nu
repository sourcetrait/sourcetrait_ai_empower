...
#
# The packet <colony_nom>_<tx_id>.txt must already be in the queen's per-sender
# packet dir on the fae side (open's channel_output_dir). Appends "COLONY SYN
# <packet>" to the fae's shared control file. response_to_rx_id is optional (null
# = plain SYN); when set it is the rx_id of a fae packet the queen received,
# adding "RE <fae_nom>_<rx_id>.txt" (correlation, not an ack). Errors if the fae
# is not online, the sent packet is missing, or the referenced received packet is
# missing in the queen's own packet dir. Void return.
export def main [args: record<fae: string, tx_id: string, response_to_rx_id: oneof<string, nothing>>]: nothing -> nothing {
    ...
    let packet_path = (common fae-queen-packet-dir $args.fae $fae_session_nom | path join $packet)
    ...
    let line = if $args.response_to_rx_id == null {
        ...
        let re_path = (common queen-input-base $colony_identity $colony_session_nom | path join "input" $re_packet)
