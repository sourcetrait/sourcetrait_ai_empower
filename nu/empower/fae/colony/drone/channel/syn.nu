...
#
# The packet <fae_nom>_<tx_id>.txt must already be in the drone's own inbox dir
# (the queen's drone-open channel_input_dir). Ensures the fae's per-drone receive
# inbox exists, then appends "FAE DRONE <name> SYN <packet>" to the drone's own
# control file. response_to_rx_id optional; when set, adds "RE <colony_nom>_<rx_id>.txt"
...
export def main [args: record<ai_identity: string, drone_name: string, tx_id: string, response_to_rx_id: oneof<string, nothing>>]: nothing -> nothing {
    ...
    let drone_base = (common drone_inbox_base $queen_identity $queen_session_nom $args.drone_name)
    let drone_in = ($drone_base | path join "input")
    ...
    mkdir (common fae_drone_packet_dir $args.ai_identity $fae_session_nom $args.drone_name)
    let drone_control = ($drone_base | path join "input.txt")
    let line = if $args.response_to_rx_id == null {
        ...
        let re_path = (common fae_drone_packet_dir $args.ai_identity $fae_session_nom $args.drone_name | path join $re_packet)
    ...
    $"($line)(char nl)" | save --append $drone_control
}
