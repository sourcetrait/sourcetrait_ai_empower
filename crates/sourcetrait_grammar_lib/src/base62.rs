...
///
/// `pub` so the id newtypes that moved out of this crate when lib_grammar
/// dissolved (`sourcetrait_grammar_mcp`'s `Nonce` / `RerunHash`,
/// `sourcetrait_grammar_claudeline`'s `ClaudeSessionNom`) render through one shared
