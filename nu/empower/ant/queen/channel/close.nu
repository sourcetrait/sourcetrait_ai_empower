...
# fae's shared control file only if it exists. Errors if the colony itself has no
...
export def main [args: record<fae: string>]: nothing -> nothing {
    ...
    let in_file = (common queen-input-base $colony_identity $colony_session_nom | path join "input.txt")
