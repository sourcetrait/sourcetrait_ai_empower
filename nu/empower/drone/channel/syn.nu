...
#
# The packet <session_nom>_<tx_id>.txt must already be written into the fae's
# per-drone packet dir (open's channel_output_dir). Appends "DRONE SYN <packet>"
# to the fae's per-drone control file.
#
...
#
# Errors if the bonded fae is not online (its per-drone inbox is absent), the
# sent packet does not exist, or (when response_to_rx_id is set) the referenced
# received packet does not exist in the drone's own packet dir.
export def main [args: record<fae: string, session_nom: string, drone_name: string, tx_id: string, response_to_rx_id: oneof<string, nothing>>]: nothing -> nothing {
    ...
    let packet = $"($args.session_nom)_($args.tx_id).txt"
    ...
    let line = if $args.response_to_rx_id == null {
        ...
        let re_path = (common drone-input-base $args.fae $args.session_nom $args.drone_name | path join "input" $re_packet)
