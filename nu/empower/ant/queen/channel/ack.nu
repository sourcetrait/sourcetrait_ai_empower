...
# Appends "COLONY ACK <packet>" to the fae's shared control file. Errors if the
...
export def main [args: record<fae: string, rx_id: string>]: nothing -> nothing {
    ...
    let packet_path = (common queen-input-base $colony_identity $colony_session_nom | path join "input" $packet)
