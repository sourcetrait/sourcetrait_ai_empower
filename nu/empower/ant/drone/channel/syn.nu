...
# SYN <packet>" to the fae's shared control file. response_to_rx_id optional;
# when set, adds "RE <fae_nom>_<rx_id>.txt" referencing a fae packet the drone
# received (in its own packet inbox). Errors if the fae is not online, the sent
# packet is missing, or the referenced received packet is missing. Void return.
export def main [args: record<fae: string, drone_name: string, tx_id: string, response_to_rx_id: oneof<string, nothing>>]: nothing -> nothing {
    ...
    let packet_path = (common fae-drone-packet-dir $args.fae $fae_session_nom $args.drone_name | path join $packet)
