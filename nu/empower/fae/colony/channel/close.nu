...
# control file only if it exists. Errors if the fae itself has no live session.
# Void return.
export def main [args: record<ai_identity: string>]: nothing -> nothing {
    ...
    if $queen_session_nom != null {
        let q_file = (common queen-input-base $queen_identity $queen_session_nom | path join "input.txt")
