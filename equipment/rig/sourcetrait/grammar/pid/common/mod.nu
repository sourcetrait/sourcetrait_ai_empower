# Shared pid/liveness derivation for the grammar:pid helpers.
#
...
def status_session [identity: string]: nothing -> oneof<record<session_nom: string, pid: int>, nothing> {
    let yaml = ($env.XDG_CACHE_HOME | path join "sourcetrait" "grammar" "claudeline" $identity "status" "latest.yaml")
    if ($yaml | path exists) {
