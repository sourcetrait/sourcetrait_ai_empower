## protocol: iter_working 

Each Iter topic (`./iter/<topic>`) should have a `working` sub-directory
that contains the entire working knowledge for that topic, documented
by Claude.
...
acts purely as a directory-wide `ragref` for the entire working knowledge of a
...
The working knowledge must be updated whenever new information concerning that
...
Topic-specific information should be captured in 'working' instead of memory,
as memory is intended for broad harness-wide information.

Working knowledge should always reflect current truth. Historical meta
