...
//!
//! Verifies:
//!   - First startup creates `<XDG_DATA_HOME>/nu_sh_mcp/keypair/{id_nu_sh_mcp,
//!     id_nu_sh_mcp.pub, allowed_signers}` and the git repo at
//!     `<XDG_DATA_HOME>/nu_sh_mcp/libraries/` with a signed initial commit.
...
impl Host {
    ...
    fn nu_sh_mcp_data_dir(&self) -> PathBuf {
        self.data_dir.path().join("nu_sh_mcp")
    }

    fn libraries_dir(&self) -> PathBuf {
        self.nu_sh_mcp_data_dir().join("libraries")
    }

    fn keypair_dir(&self) -> PathBuf {
        self.nu_sh_mcp_data_dir().join("keypair")
    }
...
fn substrate_initializes_on_first_startup() {
    ...
    assert!(
        keypair.join("id_nu_sh_mcp").exists(),
        "private key should exist at {}",
        keypair.join("id_nu_sh_mcp").display(),
    );
    assert!(
        keypair.join("id_nu_sh_mcp.pub").exists(),
...
fn register_library_writes_repo_and_mirror() {
    ...
    let meta_path = lib_dir.join(".nu_sh_mcp_meta.json");
