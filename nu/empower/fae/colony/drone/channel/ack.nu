...
# dir. Appends "FAE DRONE <name> ACK <packet>" to the drone's own control file.
# Errors if the colony is not online or the received packet does not exist. Void return.
export def main [args: record<ai_identity: string, drone_name: string, rx_id: string>]: nothing -> nothing {
    ...
    let drone_control = (common drone_inbox_base $queen_identity $queen_session_nom $args.drone_name | path join "input.txt")
    let packet = $"($queen_session_nom)_($args.rx_id).txt"
    let packet_path = (common fae_drone_packet_dir $args.ai_identity $fae_session_nom $args.drone_name | path join $packet)
    if not ($packet_path | path exists) {
        error make { msg: $"received packet does not exist: ($packet_path)" }
    }
    $"FAE DRONE ($args.drone_name) ACK ($packet)(char nl)" | save --append $drone_control
}
