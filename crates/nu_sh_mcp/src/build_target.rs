...
/// What: which build of nu_sh_mcp this binary represents -- production
...
/// calls `run_server(BuildTarget::Main)`; `src/bin/nu_sh_mcp_test.rs`
...
impl BuildTarget {
    ...
    /// `"nu_sh_mcp"` for `Main`, `"nu_sh_mcp_test"` for `Test`. Both
    ...
    pub(crate) fn name(self) -> &'static str {
        match self {
            Self::Main => lib_empower::consts::NU_SH_MCP,
            Self::Test => lib_empower::consts::NU_SH_MCP_TEST,
        }
    }
...
/// `build_target()`.
///
/// Why: the two binary entry points (`nu_sh_mcp` + `nu_sh_mcp_test`)
