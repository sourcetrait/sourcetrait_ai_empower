...
# (sourcetrait/fae:colony/drone/comm).
#
...
# fae's inbox (sourcetrait/fae:colony/comm), with packets in the fae's
...
export def session_nom [identity: string]: nothing -> oneof<string, nothing> {
    let ctx = ($env.XDG_CACHE_HOME | path join "sourcetrait" "grammar" "claudeline" $identity "context" "latest.yaml")
    if ($ctx | path exists) {
