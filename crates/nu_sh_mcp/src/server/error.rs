...
/// response with `structuredContent.error.kind == "<X>::<Y>"`.
///
/// Why: this is the single seam that converts a typed `Error` into
/// the wire shape Claude Code's CLI renders visibly (verified
/// 2026-06-02: JSON-RPC errors and `is_error=true` are both
/// CLI-dropped; success-shape with structured payload renders on
/// click-expand). Centralizing the conversion keeps every handler
/// consistent.
///
