...
# ai_identity (basename, except a colony worktree .../ant/colony/<fae> -> ant_<fae>)
...
# an ai_identity from a process cwd: the basename, except a colony worktree
# (home-relative .../ant/colony/<fae>) maps to ant_<fae>. Mirrors claudeline's
# ai_identity so a live process matches the identity its status yaml is scoped to.
...
export def live_sessions []: nothing -> table<kind: string, ai_identity: string, session_nom: string, pid: int> {
    ps -l
    ...
    | each {|proc|
        ...
        if ($status != null) and ($status.pid == $proc.pid) {
            { kind: "claude", ai_identity: $identity, session_nom: $status.session_nom, pid: $proc.pid }
        } else {
            null
        }
    }
