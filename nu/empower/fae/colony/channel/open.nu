...
#
# Creates the fae's own inbox - the single shared control file (which the fae
# monitors) plus the queen's per-sender packet dir - and appends FAE ONLINE to it,
# then (passive) to the queen's control file only if it already exists. Returns
# the fae's own input paths (channel_input_file is the shared inbox to monitor),
# and queen_online: null when the queen has no live session, else its session_nom
# plus the queen's control file (channel_output_file) and packet dir
# (channel_output_dir). Errors if the fae itself has no live session.
export def main [args: record<ai_identity: string>]: nothing -> record<channel_input_file: string, channel_input_dir: string, queen_online: oneof<nothing, record<session_nom: string, channel_output_file: string, channel_output_dir: string>>> {
    ...
    let in_file = (common fae-control-file $args.ai_identity $fae_session_nom)
    let in_dir = (common fae-queen-packet-dir $args.ai_identity $fae_session_nom)
    mkdir $in_dir
