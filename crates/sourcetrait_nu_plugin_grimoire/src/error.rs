#[allow(unused)]
pub type NuPluginGrimoireResult<T> = Result<T, NuPluginGrimoireError>;

#[derive(Debug, snafu::Snafu)]
pub enum NuPluginGrimoireError {
...
pub(crate) fn nu_plugin_error(message: impl Into<String>) -> NuPluginGrimoireError {
    NuPluginGrimoireError::Whatever {
...
impl From<NuPluginGrimoireError> for nu_protocol::LabeledError {
    fn from(err: NuPluginGrimoireError) -> Self {
...
///
/// Where: returned by `md::find`; the `grimoire eye md find` command maps it
...
/// `md::find` call sites. Moved here from `sourcetrait_grammar_lib` when
/// lib_grammar dissolved to its base62 + consts remainder.
///
