...
impl Namepath {
    ...
    pub(crate) fn validate(&self) -> Result<NamepathRef, Error> {
        ...
        let bad = |reason: &str| Error::NamepathInvalid {
            namepath: raw.to_string(),
            reason: reason.to_string(),
        };
        if raw.is_empty() {
            return Err(bad("empty namepath"));
        }
        let parts: Vec<&str> = raw.split(':').collect();
        match parts.as_slice() {
            [library] => {
                check_library(library).map_err(bad)?;
            ...
            [library, module_path] => {
                check_library(library).map_err(bad)?;
                check_module_path(module_path).map_err(bad)?;
            ...
            [library, module_path, name] => {
                ...
                if !is_valid_ident(name) {
                    return Err(bad("function name must match [a-zA-Z_][a-zA-Z0-9_-]*"));
                }
                if *name == "main" {
                    return Err(bad("`main` is reserved and cannot be a function name"));
                }
