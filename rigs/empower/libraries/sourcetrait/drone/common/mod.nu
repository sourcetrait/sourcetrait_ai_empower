...
# sourcetrait/queen:drone/comm:{open_channel,close_channel} to manage the
# drone's channel; the drone invokes comm:{ready,done,syn,ack} for its
# lifecycle and outbound. The drone has no inbox file and does not monitor -
# the queen relays inbound lines to it. The drone reads fae-sent packets from its input
