...
/// Result alias for the bed crate, with `BedError` baked in as the error
/// variant (the `std::io::Result<T>` convention).
pub type BedResult<T> = Result<T, BedError>;
...
pub enum BedError {
    #[snafu(display("could not build the bed engine: {message}"))]
