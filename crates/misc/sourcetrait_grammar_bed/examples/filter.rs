//! The two bed channels end to end: a `$in` pipeline filter from source,
//! and a `main`-with-parameters script file.
//!
//! Run: `cargo run -p sourcetrait_grammar_bed --example filter`

use sourcetrait_grammar_bed::{Bed, BedConfig, Value, record};
