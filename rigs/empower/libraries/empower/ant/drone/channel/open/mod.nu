...
export def main [args: record<fae: string, drone_name: string, persist: bool>]: nothing -> record<colony_channel_input_dir: string, colony_channel_output_dir: string, colony_channel_outbox: string> {
    ...
    let in_dir = (empower ant drone channel common drone_input_dir $colony_identity $colony_session_nom $args.drone_name)
    mkdir $in_dir
    let out_dir = (empower ant drone channel common drone_output_dir $args.fae $fae_session_nom $args.drone_name)
    let outbox = (empower ant drone channel common colony_outbox $args.fae $fae_session_nom)
    if not $args.persist {
        empower ant drone channel common announce_drone $args.fae $args.drone_name $colony_identity $colony_session_nom $fae_session_nom "ONLINE"
    }
