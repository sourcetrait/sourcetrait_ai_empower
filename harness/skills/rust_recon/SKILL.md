...
2. **§2 core vocabulary** — confirm and describe the core types other crates speak in.
3. **§3 seam-spine** — trace the dominant pattern UP TO each seam and stop.
4. **§4 data-flow narrative** — how the core data type moves entry → result.
5. **§5 worked slice** — the protagonist. Trace the one seeded instance across every boundary
   it touches. This becomes the template for authoring new instances.
6. **§7 pattern-authoring guide** — the minimal checklist to add a new instance of the
   dominant pattern.
...
running it, fold any resolved counts and disagreements into §5/§6 and flip the provenance
