...
fn analyze_target(to: &Path, protected: &[&str]) -> NuPluginEmpowerResult<Target> {
    if !to.exists() {
...
fn new_build_dir(replace: &Replace) -> NuPluginEmpowerResult<tempfile::TempDir> {
...
fn new_retire_dir(to: &Path, replace: &Replace) -> NuPluginEmpowerResult<PathBuf> {
...
fn move_path(src: &Path, dest: &Path) -> NuPluginEmpowerResult<()> {
