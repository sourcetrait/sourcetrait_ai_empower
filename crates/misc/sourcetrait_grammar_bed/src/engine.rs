...
///
/// Fully configured through `BedConfig`. The command surface is the curated
...
impl Bed {
    ...
    pub fn new(config: BedConfig) -> BedResult<Self> {
        Ok(Self {
    ...
    ///   `BedError::MissingMain`) and `input` feeds the block's first
    ///   pipeline (`$in`).
    ///
...
/// `main $__bed_arg_0 ...` call is parsed against the merged engine, so
...
fn run_main(
    ...
    let (block, delta) = {
        ...
        for (index, value) in args.into_iter().enumerate() {
            let var_name = format!("$__bed_arg_{index}");
