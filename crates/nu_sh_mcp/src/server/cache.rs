...
/// name is `"nu_sh_mcp"` on Main and `"nu_sh_mcp_test"` on Test, per
/// `build_target().name()`.
///
...
/// (slice 3 substrate). The target name is `"nu_sh_mcp"` on Main and
/// `"nu_sh_mcp_test"` on Test, per `build_target().name()`.
///
