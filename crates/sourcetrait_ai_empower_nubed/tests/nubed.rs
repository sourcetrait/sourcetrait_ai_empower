use indoc::indoc;
use sourcetrait_ai_empower_nubed::{
    NuBed, NuBedConfig, NuBedError, NuBedResult, Value, record,
};
...
fn bed() -> NuBed {
    NuBed::new(NuBedConfig::default()).expect("build NuBed")
}

/// Run source through a fresh default bed with no input and no args.
fn run(source: &str) -> NuBedResult<Value> {
...
fn assert_denied(
    what: &str,
    result: NuBedResult<Value>,
) {
    match result {
        Err(NuBedError::Parse { .. })
        | Err(NuBedError::Compile { .. })
        | Err(NuBedError::Eval { .. }) => {}
...
fn args_without_main_error() {
    ...
    match result {
        Err(NuBedError::MissingMain { count, .. }) => assert_eq!(count, 1),
