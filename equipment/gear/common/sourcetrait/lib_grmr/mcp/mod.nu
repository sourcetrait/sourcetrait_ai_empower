# grammar_mcp administration from the user's shell (`grmr mcp ...`).
#
# Wraps `grammar_mcp [--id ..] [--namespace ..] [--workdir ..] cli <tool> ...`
# and parses the
...
# user reaches grammar_mcp through `grmr mcp`, and the agent drives the MCP
...
def mcp_cli [id: string, namespace: string, workdir: string, cli_args: list<string>]: nothing -> oneof<record, nothing> {
    ...
    let outcome = (^grammar_mcp ...$argv | complete)
    let raw = ($outcome.stdout | str trim)
    let parsed: oneof<record, nothing> = (if ($raw | is-empty) { null } else { $raw | from json })
    if $outcome.exit_code != 0 {
        error make { msg: $"grammar_mcp cli failed: (mcp_error_detail $parsed $outcome.stderr)" }
    }
    $parsed
}
