...
# its own control file, and (passive) to the fae's shared control file only if it
# already exists. Returns the queen's own input paths, and fae_online: null when
# the fae has no live session, else its session_nom plus the fae's shared control
# file (channel_output_file) and the queen's per-sender packet dir
# (channel_output_dir). Errors if the colony itself has no live session.
export def main [args: record<fae: string>]: nothing -> record<channel_input_file: string, channel_input_dir: string, fae_online: oneof<nothing, record<session_nom: string, channel_output_file: string, channel_output_dir: string>>> {
    ...
    let in_base = (common queen-input-base $colony_identity $colony_session_nom)
    ...
    let fae_online = if $fae_session_nom == null {
        ...
        let fae_packets = (common fae-queen-packet-dir $args.fae $fae_session_nom)
