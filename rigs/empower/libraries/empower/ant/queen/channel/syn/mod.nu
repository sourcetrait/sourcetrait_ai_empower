...
export def main [args: record<fae: string, tx_id: int, response_to_rx_id: oneof<int, nothing>>]: nothing -> nothing {
    ...
    let packet_path = (empower ant queen channel common queen_output_dir $args.fae $fae_session_nom | path join $packet)
    ...
    let line = if $args.response_to_rx_id == null {
        ...
        let re_path = (empower ant queen channel common queen_input_dir $colony_identity $colony_session_nom | path join $re_packet)
