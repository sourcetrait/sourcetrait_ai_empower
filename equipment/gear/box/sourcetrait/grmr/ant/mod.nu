...
# Ported verbatim from the retired grmr bin's `main ant *` commands (the
...
export def new [ant_dir: directory, harness_templates_dir?: directory]: nothing -> nothing {
    ...
    let harness_templates_dir = if ($harness_templates_dir == null) {
        ...
        error make {msg: "Grammar harness template directory not found"}
    } else {
...
def "report info" []: string -> nothing {
    print $"(ansi blue)[grmr](ansi reset) ($in)"
}

def "report warn" []: string -> nothing {
    print $"(ansi yellow)[grmr](ansi reset) ($in)"
}
...
def find_harness_templates_dir []: nothing -> oneof<directory, error> {
    let dir: directory = ($env.HOME | path join 'proj/sourcetrait/sourcetrait_grammar/harness')
    if ($dir | path exists) { return $dir }

    let dir: directory = /usr/local/share/sourcetrait/grammar/harness
    if ($dir | path exists) { return $dir }
